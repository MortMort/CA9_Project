classdef comps < wtLin.mapAbleStruct
    ...
    methods (Static)
        function c=calcComps(lp)
            ...
            %
            c.s.PLC_Prefilt_Mess = getPLC_Prefilt_Mess(lps.mp.ctr.plc.tau_mess);
            c.s.PLC_Prefilt_Ref = getPLC_Prefilt_Ref(lps.mp.ctr.plc.tau_ref);
            c.s.PLC=getPLC(lps.mp.ctr.plc);
            c.s.FLC_Prefilt_Ref = getFLC_Prefilt_Ref(lps.mp.ctr.flc.tau_ref);
            ...
            %c.s.drtFree2nac=getDrivetrainDynFree2Nac(lps.mp.drt);
            %
            ...
            %
            if lps.mp.cnv.GAPC_format_version > 0 %GAPC Converter
                c.s.cnvgenDtd = getConverterGeneratorDtd(lps.mp.cnv,lps.info.gen,lps.stat);
            end
            %
            c.s.pit=getPitchDyn(lps.mp.pit);
            c.s.pit2=getPitch2Dyn();
            ...
            %
            %c.s.nac = getNacdyn(lps.mp.ctr.nac);
...
%GAPC Converter model including Generator gain with DTD input
function dyn=getConverterGeneratorDtd(cnv,gen,stat)
    ...
    % u3 = w [RPM]
    Rpm2RadSec = pi/30;
    tau = 1/(cnv.GAPC.LPf*2*pi);

    dStpRefdPowRef = 1 / (stat.genSpd*Rpm2RadSec * gen.etaE * gen.etaM * gen.etaA);
    dStPRefdw = -stat.power / ((stat.genSpd*Rpm2RadSec)^2 * gen.etaE * gen.etaM * gen.etaA);
    
    F = -1/tau;
    G = [1/tau 1/tau 0];
    H = dStpRefdPowRef;
    J = [0 0 dStPRefdw*Rpm2RadSec];
    
    dyn = ss(F,G,H,J);
    
    dyn.InputName = {'Pref','Pdtd','w'};
    dyn.OutputName = {'Mgen'};
...
%Simplified Pitch dynamics
function dyn=getPitch2Dyn()   

    %Temp Pitch model, not verified for generic variants
    pit.tau1 = 1/1.2 / (2*pi);
    pit.tau2 = 1/3.5 / (2*pi);

    F = [-1/pit.tau1     0;
         1/pit.tau2 -1/pit.tau2]; 
    
    G = [1/pit.tau1 1/pit.tau1;
             0           0   ];
    H = [0 1];
    J = [0 0]; 
    dyn = ss(F,G,H,J);
...
% PLC Pre filter on measured generator speed
function dyn=getPLC_Prefilt_Mess(tau)
   dyn = tf(1,[tau 1]);
   dyn.InputName = 'w';
   dyn.OutputName = 'wFilt';
end

% PLC Pre filter on generator speed references
function dyn=getPLC_Prefilt_Ref(tau)
   dyn = tf(1,[tau 1]);
   dyn.InputName = 'wRef';
   dyn.OutputName = 'wRefFilt';
end

% FLC Pre filter on generator speed references
function dyn=getFLC_Prefilt_Ref(tau)
   dyn = tf(1,[tau 1]);
   dyn.InputName = 'wRef';
   dyn.OutputName = 'wRefFilt';
end
