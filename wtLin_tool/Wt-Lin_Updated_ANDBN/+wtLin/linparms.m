...
function [mp,info]=getLinParams(gp,op,stat)
    ...
    %% PLC Pre filter
    mp.ctr.plc.tau_ref = g.ctr.plc.tau_ref;
    mp.ctr.plc.tau_mess = g.ctr.plc.tau_mess;
    ...
    if g.cnv.GAPC_format_version > 0
        mp.cnv.GAPC_format_version = g.cnv.GAPC_format_version;
        mp.cnv.GAPC.LPf=g.cnv.GAPC.LPf;
    else
        mp.cnv.GAPC_format_version = -1;
    end        
    ...
    %% FLC
    % Gain adaptation vs operating point 
    K = g.ctr.flc.OpGainSchK;
    ThetaMin = g.ctr.flc.OpGainSchThetaMin;
    ThetaGS = g.ctr.flc.OpGainSchTheta;
    if (ThetaGS > ThetaMin)
    ...
    if g.ctr.flc.OpPointEstimatorType == 0
        OpPointPitch = stat.pitch ; %[deg]
    else
        warning('Legacy operation point estimates are not supported. Pitch position is used to calculate the operation point');
        OpPointPitch = stat.pitch; %[deg]
    end
    Op_GainSch = min( max(offset + slope * OpPointPitch,1) , g.ctr.flc.OpGSMax);
    
    % Online / Linear Moment
    [pitGS,pitLinGS,pitSensGS] = wtLin.ThetaGS(g,o,stat,stat.pitch);
    windGS = wtLin.WindGS(stat.pitch,g.ctr.flc);
    GS = pitGS * windGS * Op_GainSch;
    ...
    mp.ctr.flc.tau_ref = g.ctr.flc.tau_ref;
