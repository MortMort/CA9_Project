function [out_struct] = TurnerGearExtremeLoadsMatrix_ClockWise(DataMaxBin,DataMinBin,varargin)
    ...
    Max2B_positive = DataMaxBin(:,6);
    Min2B_negative = DataMinBin(:,6);
    ...
    Max2B_2sf_positive = DataMaxBin(:,13);
    Min2B_2sf_negative = DataMinBin(:,13);
    ...
    Max2B_60sf_positive = DataMaxBin(:,20);
    Min2B_60sf_negative = DataMinBin(:,20);
    ...
    filename = fullfile(OutputFolder,'Results_Clockwise');
    wb.SaveAs(filename);
    wb.Close;
    h.Quit;
    h.delete;
    ...
    function [] = WriteDataExcelSheet(h,Azim,Yaw,data,SheetName,NBlades,line)
        ...
        switch NBlades
            case 1 % If only one blade (Blade 1)
                ...
                index = find((Azim>=270) & (Azim<=360));
                cell9 = get (h.Activesheet.Cells,'Item',length(Yaw)+4,index(1)+1);
                cell10 = get (h.Activesheet.Cells,'Item',length(Yaw)+4,index(end)+1);
                ...
                set(ActiveRange, 'Value',  ['=''',SheetName,'''!',get(cell9,'Address')], 'NumberFormat',  '0.0');
    ...
    function [] = PrepareResultsSheet(h)
        ...
        ActiveRange = get(h.Activesheet,'Range','A1');
        set(ActiveRange, 'Value',  'Turner gear loads - rotor is turned clockwise');
