function VSCoutToVTSclm(Outfile, extendbins,Component)
...
VSCOut = ReadNTMFat(VSCOut,C,Component);
...
%% check if log normal is used
disp(' ')
disp('- Reading key word for lognormal...')
VSCOut.LognormalString = 'Keyword for log normal not found';
key = 'IEC turbulence model:';
VSCOut.LognormalString = GetValueForKeyFromClmFile(C{1} , key);
disp('Log normal key word found: ');
disp(VSCOut.LognormalString);
% if strcmp(VSCOut.LognormalString, 'IEC61400-1 ed. 3 with Wöhler dependent turbulence quantile LIMITED')>0
%     disp ('Log normal used');
% else
%    disp ('Log normal not used');
% end
...
dataToTable = GetDataAsTableFromClmFile(C{1} , key, 4, 6, -1, 1); %GetDataAsTableFromClmFile(outFileAsChar , key, nRowsToSkip, nColumnsToSkip, nRowsToRead, nColumnsToRead)
if length(unique(dataToTable))>1
...
%% Read Turbine Label and turbulence of worst fatigue loaded turbine depending on input
% This is for NTM_Fat...
function VSCOut = ReadNTMFat(VSCOut,C,Component)
% User input
...
switch lower(Component)
    ...
    case 'e'
        ...
        lineToRead = pos + skipLines;
        Dummy = textscan(C{1}{lineToRead},'%*s WTG No. %15c WTG No. %15c WTG No. %15c WTG No. %15c');
        if isempty(Dummy{1}) % 1.5.6 Updated
            Dummy = textscan(C{1}{lineToRead},'%*s %15c %15c  %15c  %15c');
        end
        Dummy = strtrim(Dummy);
...
if isempty(pos)
    VSCOut.keyVe50 = 'Static gust wind speed (3 sec. mean)';
    pos = GetPosOfKeyFromClmFile(C{1} , VSCOut.keyVe50);
end
...
fprintf(fid,'Created by VSCoutToVTSclm.m MatLab-Script V01.00.\n');
fclose(fid);
