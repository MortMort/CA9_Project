...
% Val2str subfunction bugfix by H. Gsenger, 19-9-2011
    ...
    if(nargout == 0)
        ...
        % Workaround for bug in xmlwrite. CDATA is not written properly
        [FID, status] = fopen(file,'rt');
        if FID > 0
            filedata = textscan(FID, '%s', -1, 'whitespace', '', 'delimiter', '\n');
            fclose(FID);
            filedata = filedata{1};
            filedata = strrep(filedata,'&lt;![CDATA[','<![CDATA[');
            filedata = strrep(filedata,']]&gt;',']]>');
            [FID, status] = fopen(file,'wt');
            if FID > 0
                fprintf(FID, '%s\n', filedata{:});
                fclose(FID);
            end
        ...
        % Workaround for bug in xmlwrite. CDATA is not written properly
        varargout{1} = strrep(varargout{1},'&lt;![CDATA[','<![CDATA[');
        varargout{1} = strrep(varargout{1},']]&gt;',']]>');
    end
end
...
function [] = parseStruct(s,docNode,curNode,pName)
    ...
    for i = 1:length(fnames)
        ...
        elseif (strcmp(curfield,'CDATA'))
            % CDATA
            curNode.appendChild(docNode.createCDATASection(['<![CDATA[' s.CDATA ']]>']));
        else
            %Sub-element
            if (isstruct(s.(curfield)))
                %single element
